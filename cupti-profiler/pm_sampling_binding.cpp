...
// We may need the following if you plan to handle exceptions or convert them to Python exceptions
...
public:
     ...
     *   3) Sets up the CuptiProfilerHost so that we can later create config images, query metrics, etc.
     */
    PmSampler(int deviceIndex)
      : m_deviceIndex(deviceIndex)
    {
        ...
        // Some basic device checks (optional). For example, check device support:
        {
            ...
            {
                throw std::runtime_error("PM Sampling not supported on this device configuration.");
            }
        }
        ...
        CuptiPmSampling::GetCounterAvailabilityImage(deviceIndex, m_counterAvailability);
    ...
    /**
     * query_base_metrics():
     *   - Returns a list of all base metrics reported by the CUPTI profiler for this device.
     */
     ...
     *   - For each metric name, return a dictionary of { "description": ..., "type": ... }.
     */
    ...
    query_metric_properties(const std::vector<std::string> &metricNames)
    {
        ...
        for (const auto &metric : metricNames)
        {
            ...
            switch (metricType)
            {
            case CUPTI_METRIC_TYPE_COUNTER:    metricTypeStr = "Counter";    break;
            case CUPTI_METRIC_TYPE_THROUGHPUT: metricTypeStr = "Throughput"; break;
            case CUPTI_METRIC_TYPE_RATIO:      metricTypeStr = "Ratio";      break;
            default:                           metricTypeStr = "Unknown";    break;
            }
    ...
    /**
     * enable_sampling(metrics, sampling_interval, hardware_buffer_size, max_samples):
     *   1) Create the config image for the given set of metrics.
     *   2) Initialize and enable PM sampling for the device.
     *   3) Set the config (interval, buffer size, etc.).
     *   4) Create the counter data image that will store the samples.
     */
                         ...
                         uint64_t maxSamples       = 10000)
    {
        ...
        m_pmSampler.CreateCounterDataImage(maxSamples, m_metricPointers, m_counterDataImage);
    ...
    {
        if (!m_samplingEnabled)
            throw std::runtime_error("Sampling has not been enabled. Call enable_sampling() first.");
    ...
    /**
     * get_samples():
     *   - Decode the data in the counterDataImage, retrieving all completed samples.
     *   - For each completed sample, EvaluateCounterData and build a Python-friendly structure:
     *       {
     *           "startTimestamp": <uint64_t>,
     *           "endTimestamp":   <uint64_t>,
     *           "metrics":        { "metric_name": <value>, ... }
     *       }
     *   - Then reset (so that future calls to get_samples() will fetch newly accumulated samples).
     *   - Return a list of samples.
     *
     * Note: For a "continuous" scenario you'd typically call decode repeatedly
     *       (perhaps on another thread) to not overflow the internal hardware buffer.
     */
    std::vector<py::dict> get_samples()
    {
        ...
        if (status != CUPTI_SUCCESS)
        {
            ...
            throw std::runtime_error(std::string("cuptiPmSamplingGetCounterDataInfo failed: ") + errstr);
        }
        ...
        for (size_t sampleIndex = 0; sampleIndex < counterDataInfo.numCompletedSamples; ++sampleIndex)
        {
            ...
            // but we also need to retrieve timestamps from cuptiPmSamplingCounterDataGetSampleInfo.
...
private:
    ...
    // (We rely on CuptiProfilerHost having a small accessor or store the pointer as needed.)
    CUpti_Profiler_Host_Object* m_profilerHostObject()
    {
        // We'll do a small hack: we rely on CuptiProfilerHost storing it in a private field
        // but we have a small accessor if needed. Let's add a small method:
...
private:
    ...
    // A copy of user-provided metric names and pointers (lifetime must outlast usage).
...
PYBIND11_MODULE(pm_sampling, m)
{
    ...
    py::class_<PmSampler>(m, "PmSampler")
        ...
        .def("query_base_metrics",
             &PmSampler::query_base_metrics,
             R"pbdoc(
                Return a list of all supported base metrics on this device.
             )pbdoc")
        .def("query_metric_properties",
             &PmSampler::query_metric_properties,
             py::arg("metric_names"),
             R"pbdoc(
